...
cur =conn.cursor()
temp_conn = MySQLdb.connect('localhost', 'root', '123456', 'jol', charset='utf8')
...
#re_sdutoj = re.compile(r'<td>Solved<td align=center><a href[\s\S]*?>(\d+)</td>')
...
def sdutoj_solved(user):
    if user[12]:
        temp_cur.execute('select solved from users where user_id="%s"' % user[12])
        solved = temp_cur.fetchall()
        solved = solved[0]
        #if user_id:
           # user_id = user_id[0]
...
def oj_solved(user, info):
    #print info[1],user[info[1]],info[2] % user[info[1]]
    if user[info[1]]:
        userstatusurl = info[2] % user[info[1]]
        html = get_html(userstatusurl)
        solved = info[3].findall(html)
        print info[0], solved, info[1]
        if solved:
