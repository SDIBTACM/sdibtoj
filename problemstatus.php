...
<?
echo "<center><h1>Problem <a href=./problem.php?id=$id>$id</a> Status</h1></center>";
...
echo "<tr bgcolor=#D7EBFF><td>$MSG_SUBMIT<td><a href=status.php?problem_id=$id>".$row[0]."</tr>";
$total=intval($row[0]);
...
for ($i=$start+1;$row=mysql_fetch_object($result);$i++){
	...
	echo "<td>$i";
		if (intval($row->att)>1)
          {
              echo "<td><a href=status.php?problem_id=$id&user_id=$row->user_id&language=-1&jresult=4&showsim=0>$row->solution_id";
              echo "(".$row->att.")";
          }
        else
